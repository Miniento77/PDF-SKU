...
`label-pdf-sku` 是一个本地网页工具，一次处理一张快递面单 PDF。它会尽量保留原始 PDF 的矢量内容，把任意单页输入适配到 4:6 输出比例，并在页面下方追加 SKU 与数量区域，方便面单打印。
...
- 使用 `pypdf` 读取单页输入 PDF。
- 根据输入页原始宽高构造 4:6 目标页，并以矢量级页面变换把原始页面等比例放入目标页，不裁切、不拉伸。
